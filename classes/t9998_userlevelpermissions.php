...
/**
 * Table class for t9998_userlevelpermissions
 */
class t9998_userlevelpermissions extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't9998_userlevelpermissions';
		$this->TableName = 't9998_userlevelpermissions';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t9998_userlevelpermissions`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// userlevelid
		$this->userlevelid = new DbField('t9998_userlevelpermissions', 't9998_userlevelpermissions', 'x_userlevelid', 'userlevelid', '`userlevelid`', '`userlevelid`', 3, -1, FALSE, '`userlevelid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tablename
		$this->_tablename = new DbField('t9998_userlevelpermissions', 't9998_userlevelpermissions', 'x__tablename', 'tablename', '`tablename`', '`tablename`', 200, -1, FALSE, '`tablename`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// permission
		$this->permission = new DbField('t9998_userlevelpermissions', 't9998_userlevelpermissions', 'x_permission', 'permission', '`permission`', '`permission`', 3, -1, FALSE, '`permission`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->permission->Nullable = FALSE; // NOT NULL field
		$this->permission->Required = TRUE; // Required field
		$this->permission->Sortable = TRUE; // Allow sort
		$this->permission->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`t9998_userlevelpermissions`";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if ($rs) {
			...
			if (array_key_exists('tablename', $rs))
				AddFilter($where, QuotedName('tablename', $this->Dbid) . '=' . QuotedValue($rs['tablename'], $this->_tablename->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('userlevelid', $row) ? $row['userlevelid'] : NULL) : $this->userlevelid->CurrentValue;
		if (!is_numeric($val))
		...
		$val = is_array($row) ? (array_key_exists('tablename', $row) ? $row['tablename'] : NULL) : $this->_tablename->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@_tablename@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "t9998_userlevelpermissionslist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "t9998_userlevelpermissionsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "t9998_userlevelpermissionsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t9998_userlevelpermissionsadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t9998_userlevelpermissionsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t9998_userlevelpermissionsview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "t9998_userlevelpermissionsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "t9998_userlevelpermissionsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t9998_userlevelpermissionsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t9998_userlevelpermissionsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t9998_userlevelpermissionsdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "userlevelid:" . JsonEncode($this->userlevelid->CurrentValue, "number");
		$json .= ",_tablename:" . JsonEncode($this->_tablename->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->userlevelid->CurrentValue != NULL) {
			$url .= "userlevelid=" . urlencode($this->userlevelid->CurrentValue);
		} else {
			return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
		}
		if ($this->_tablename->CurrentValue != NULL) {
			$url .= "&_tablename=" . urlencode($this->_tablename->CurrentValue);
		} else {
			return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
		}
		return $url;
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("userlevelid") !== NULL)
				$arKey[] = Param("userlevelid");
			...
			if (Param("_tablename") !== NULL)
				$arKey[] = Param("_tablename");
	...
	{
		$table = 't9998_userlevelpermissions';
		$usr = CurrentUserID();
