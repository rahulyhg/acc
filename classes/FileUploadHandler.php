...
class UploadHandler extends \UploadHandler
{
	...
	{
		$id = UPLOAD_TEMP_FOLDER_PREFIX . $this->SessionId;
		if ($this->UploadId <> "") {
			$uid = $this->UploadId;
			if ($this->UploadTable <> "")
	...
	{
		if (UPLOAD_CONVERT_ACCENTED_CHARS) {
...
class FileUploadHandler
{
	...
	public function run()
	{
		...
		$exts = Param("exts", "");
		$arExt = explode(",", $exts);
		if (UPLOAD_ALLOWED_FILE_EXT <> "") {
			$arAllowedExt = explode(",", UPLOAD_ALLOWED_FILE_EXT);
			$exts = implode(",", array_intersect($arExt, $arAllowedExt)) ?: UPLOAD_ALLOWED_FILE_EXT; // Make sure $exts is a subset of UPLOAD_ALLOWED_FILE_EXT
		} elseif ($exts == "") {
		...
		$url = API_URL . "?" . API_ACTION_NAME . "=" . API_JQUERY_UPLOAD_ACTION . "&rnd=" . Random() . (($uploadId <> "") ? "&id=" . $uploadId : "") . (($uploadTable <> "") ? "&table=" . $uploadTable : "") . (($sessionId <> "") ? "&session=" . $sessionIdEncrypted : ""); // Add id/table/session for display and delete
		...
		$options = array(
			...
			"image_versions" => array(
				...
				UPLOAD_THUMBNAIL_FOLDER => array(
					"max_width" => UPLOAD_THUMBNAIL_WIDTH,
					"max_height" => UPLOAD_THUMBNAIL_HEIGHT,
					"jpeg_quality" => THUMBNAIL_DEFAULT_QUALITY,
					"png_quality" => 9
				)
			)
		);
